...
> **Skill 基础目录**：`/Users/eateralzeng/.claude/skills/flow-trace-java/`，后续简称 `<skill_dir>`。
...
```
<project>/flow-trace-java/.trace-cache/  ← cache_dir
...
│   └── merged-rmb-*.json        ← Phase 4 输出：合并后 RMB 流程
├── phase5/
│   └── {entryId}-semantics.json ← Phase 5 输出：含业务描述的数据
└── phase6/
    └── (输出在 <project>/flow-trace-java/flows/)
```

**路径变量**：
- `cache_dir` = `<project>/flow-trace-java/.trace-cache/`
- `entries_path` = `<cache_dir>/phase1a/entries.json`
- `output_dir` = `<project>/flow-trace-java/`

## 工作流程

当用户调用 `/flow-trace-java <project_path>` 时：
...
2. Read `phases/phase3-tree-expand.md` 获取详细执行逻辑
3. 对每个入口（可并发执行）：
...
**方式 1：启动时自动检测**
用户调用 `/flow-trace-java <path>` 时自动检查 progress.json。
