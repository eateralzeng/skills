...
BRIDGE_TEMPLATES = {
    ...
    "MQ":    "MQ 桥接：通过 topic={topic} 触发接收端",
    "EVENT": "事件桥接：发布 {eventClass} 事件",
}

LISTENER_TEMPLATES = {
    "MQ_LISTENER":    "MQ 接收端：监听 topic={topic}",
    "EVENT_LISTENER": "事件监听器：处理 {eventClass} 事件",
}

DISPATCH_FALLBACK = "多态分发节点（实现细节未识别）"
DISPATCH_IMPL_FALLBACK = "分发实现节点（细节未识别）"

VIRTUAL_CLASSES = ("BRIDGE", "MQ_LISTENER", "EVENT_LISTENER")
...
def _generate_di_description(node):
    """Rule 2: template lookup by (type, operation)."""
    di = node.get("domainInteraction") or {}
    di_type = di.get("type")
    operation = di.get("operation")
    key = (di_type, operation)

    template = DI_TEMPLATES.get(key)
    if not template:
...
def _generate_virtual_description(node):
    ...
    if class_name == "MQ_LISTENER":
        topic = _extract_topic(node_id) or node.get("description", "")
        return LISTENER_TEMPLATES["MQ_LISTENER"].format(topic=topic or "[未识别]")
    if class_name == "EVENT_LISTENER":
        event_class = _extract_event_class(node_id) or node.get("description", "")
        return LISTENER_TEMPLATES["EVENT_LISTENER"].format(eventClass=event_class or "[未识别]")
...
def _generate_bridge_description(node_id):
    ...
    if node_id.startswith("BRIDGE:MQ:"):
        topic = _extract_topic(node_id) or "[未识别]"
        return BRIDGE_TEMPLATES["MQ"].format(topic=topic)
    if node_id.startswith("BRIDGE:EVENT:"):
        event_class = _extract_event_class(node_id) or "[未识别]"
        return BRIDGE_TEMPLATES["EVENT"].format(eventClass=event_class)
    return "桥接节点（元数据缺失）"


def _extract_topic(node_id):
    """Extract topic from nodeId suffix after '->'."""
    if "->" in node_id:
        return node_id.rsplit("->", 1)[-1]
    return None


def _extract_event_class(node_id):
    """Extract event class from nodeId suffix after '->'."""
    if "->" in node_id:
        return node_id.rsplit("->", 1)[-1]
    return None
...
def split_into_batches(nodes, batch_size=15):
    """Split subagent nodes into batches respecting parent-child constraints.

    Guarantees:
    - No parent-child pair in same batch (connectivity constraint).
    - Parents always appear in earlier batches than children (layer ordering).
    """
    sorted_nodes = sorted(nodes, key=lambda n: (n.get("layer", 0), n.get("nodeId", "")))
    ...
    for node in sorted_nodes:
        ...
        current_batch.append(node.get("nodeId"))
        current_ids.add(node.get("nodeId"))
