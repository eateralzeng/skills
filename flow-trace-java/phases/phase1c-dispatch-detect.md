...
1. 准备验证上下文：
   ...
   ```
   输出：`phase1c/_verify-context.json`

2. 分批派发 LLM 子代理：
   - 读取 `prompts/phase1c-verify.md`
   - 对每个 batch，替换 `{{patterns}}` 和 `{{project_dir}}`
   - 派发子代理（多个 batch 可并行）
   - 保存子代理输出到 `phase1c/_verify-result-{batchIndex}.json`

3. 合并验证结果：
   - 合并所有 batch 的 results 数组为单一 JSON
   - 保存到 `phase1c/_verify-results.json`
...
4. 应用验证结果：
   ...
   - 输出 `phase1c/_verify-report.json`（记录移除的 pattern）
