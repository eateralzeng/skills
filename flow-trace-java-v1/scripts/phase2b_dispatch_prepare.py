...
"""
import json, os, argparse, sys, hashlib


def _summary_filename(short_name, dispatch_key):
    """方案A：dispatch-summary 文件名 = dispatch-summary-{短名}-{hash(dispatchKey)}.json。
    短名可读 + hash 防同短名接口文件互相覆盖（issue 4/9）。"""
    h = hashlib.md5((dispatch_key or "").encode()).hexdigest()[:8]
    return f"dispatch-summary-{short_name}-{h}.json"
...
def do_prepare(args):
    ...
    for pat in patterns:
        # 仅"明确拒绝"才 skip：verify-apply 已把 _verified=false 的误报从 pattern-index 移除，
        # 此处 _verified=false 实际不触发（保留作防御）。_verified 缺失（未跑精筛 / LLM 漏报）
        # 按 detect 结果处理，避免 phase2b 全 skip 导致 DISPATCH 调用链丢失（CR-02）。
        if pat.get("_verified") is False:
            skipped += 1
            continue
        ...
        dispatch_key = pat.get("dispatchKey", f":{interface}")  # 方案A：对接键
        context = {
            "interface": interface,
            "dispatchKey": dispatch_key,
            "shortName": short_name,
            "summaryFileName": _summary_filename(short_name, dispatch_key),  # 子代理/编排者写盘用
...
def do_merge(args):
    ...
    # Get interface / dispatchType / dispatchKey from context or pattern-index
    interface = ""
    ...
    if os.path.exists(context_path):
        ...
        dispatch_key = ctx.get("dispatchKey", "")
    else:
        pi_path = os.path.join(args.cache_dir, "phase1c", "pattern-index.json")
        if os.path.exists(pi_path):
            for p in _load_json(pi_path).get("patterns", []):
                if p["interface"].rsplit(".", 1)[-1] == pattern_name:
                    ...
                    dispatch_key = p.get("dispatchKey", "")
                    break
    if not dispatch_key:
        dispatch_key = f":{interface}"  # 兜底

    output = {
        "interface": interface,
        "dispatchKey": dispatch_key,  # 方案A：对接键
        "dispatchType": dispatch_type,
        "results": deduped,
    }

    output_path = os.path.join(phase2b_dir, _summary_filename(pattern_name, dispatch_key))
...
def do_validate(args):
    ...
    # 仅排除明确 _verified=false（已被 verify-apply 移除，实际不触发）；
    # _verified 缺失（未跑精筛 / 漏报）的 pattern 也纳入校验范围（CR-02）
    verified = [p for p in patterns if p.get("_verified") is not False]
    total = len(verified)
    missing_files = []
    incomplete = []
    ok = 0
    ...
    for pat in verified:
        interface = pat["interface"]
        short_name = interface.rsplit(".", 1)[-1]
        dispatch_key = pat.get("dispatchKey", f":{interface}")
        summary_path = os.path.join(phase2b_dir, _summary_filename(short_name, dispatch_key))  # 方案A：hash 文件名
        ...
        if data.get("dispatchKey") != dispatch_key:  # 方案A：对接键一致
            issues.append(f"dispatchKey mismatch: {data.get('dispatchKey')!r} != {dispatch_key!r}")
