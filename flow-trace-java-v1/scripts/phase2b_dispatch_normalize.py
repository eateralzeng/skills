...
def normalize_file(filepath, pattern_index):
    ...
    # Fix 1b: backfill dispatchKey from pattern-index by interface (方案A)
    if isinstance(data, dict) and not data.get("dispatchKey"):
        iface = data.get("interface", "")
        for p in pattern_index:
            if p.get("interface") == iface and p.get("dispatchKey"):
                data["dispatchKey"] = p["dispatchKey"]
                structural_fix = True
                break
