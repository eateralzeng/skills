...
def do_detect(args):
    ...
    for iface_short, impl_list in sorted(interface_map.items()):
            ...
            if impl_file:
                try:
                    with open(impl_file) as f:
                        content = f.read()  # 读全文（修复 read(2000) 截断 extends 类名 bug）
                    ext = EXTENDS_RE.search(content)
                    if ext:
                        parent = ext.group(1)
        ...
        pattern = {
            "type": dispatch_type,
            "interface": iface_fqn,
            "dispatchKey": f"{_extract_module(iface_file, project_dir)}:{iface_fqn}",  # 方案A 全局唯一键 module:package.class
    ...
    for abs_short, (abs_fqn, abs_file, abs_content, concrete_list) in sorted(extends_map.items()):
        ...
        pattern = {
            "type": dispatch_type,
            "interface": abs_fqn,
            "dispatchKey": f"{_extract_module(abs_file, project_dir)}:{abs_fqn}",  # 方案A 全局唯一键 module:package.class
...
def do_verify_prepare(args):
    ...
    os.makedirs(os.path.join(cache_dir, "phase1c", "tmp"), exist_ok=True)  # 修复 ISSUE-1c-07：写 tmp 前建目录
