...
纯脚本阶段。从 Phase 2a 的调用树中剪掉无法抵达任何终端节点的分支。
...
1. 对每个入口：
   ...
   c. 从每个终端节点用 edges **反向 BFS**（取入边列表回溯，DAG 多入边全部保留；parentId 已是 list）标记沿途节点为"保留"
...
```json
{
  ...
  "chain": ["...保留的节点列表..."],
  "prunedNodes": [
    {"nodeId": "...", "class": "...", "method": "...", "layer": 0, "layerType": "INTERNAL", "reason": "not_on_terminal_path"}
  ],
